...
def get_location(soup: BeautifulSoup) -> Dict[str, str]:
    ...
    """
    table = soup.find('table', attrs={"summary": "Principal Place of Business and Contact Information"})
...
def get_exemptions(soup: BeautifulSoup) -> List[str]:
    ...
    """
    table = soup.find('table', attrs={"summary": "Federal Exemptions and Exclusions Claimed"})
    cols = table.find_all('td')
...
def get_sales_comp(soup: BeautifulSoup) -> List[dict]:
    ...
    for table in tables:
        ...
        for i, row in enumerate(rows):
            if i % 2 == 0:
                data = None
                header = row.find_all('td', recursive="False", attrs={"class": "FormText"})
                header = [h for h in header if not h.find('table') and not h.text == 'None']
                if not header:
...
def get_offering_amt(soup: BeautifulSoup) -> Dict[str, str]:
    ...
    """
    table = soup.find('table', attrs={"summary": "Offering and Sales Amounts"}).find('tbody')
    rows = table.find_all('tr', recursive=False)
    amount = {}
    for row in rows:
        cols = row.find_all('td', recursive=False)
        elements = [strip(c.text) for c in cols[1:]]
        amount[cols[0].text] = elements[0] if 'orXIndefinite' not in elements else 'Indefinite'
    return amount
